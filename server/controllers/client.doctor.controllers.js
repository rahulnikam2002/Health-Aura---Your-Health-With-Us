        ...
        verify(userToken, process.env.SECRET_KEY, (err, decoded) => {
          if (err) {
            connection.query(
              ...
              (err, allDoctorsData) => {
                if (err) {
                  res.render("doctors-listing.hbs", {
                    title: "Doctors | HealthAura",
                    message: "Something went wrong try to refress",
                  });
                } else {
                  if (allDoctorsData.length == 0) {
                    res.render("doctors-listing.hbs", {
                      ...
                      noCity: true,
                      message:
                        "No data found for this area, Try to search any nearby area",
                    });
                  } else {
                    const numOfResults = allDoctorsData.length;
                    const numberOfPages = Math.ceil(
                      numOfResults / resultPerPage
                    );
                    let page = req.query.page ? Number(req.query.page) : 1;

                    const startingLimit = (page - 1) * resultPerPage;
                    connection.query(
                      `SELECT * FROM healthaura_doctors LIMIT ${startingLimit},${resultPerPage}`,
                      (err, allDoctorsData) => {
                        if (err) {
                          res.render("doctors-listing.hbs", {
                            title: "Doctors | HealthAura",
                                ...
                                (err, allDoctorsData) => {
                                  if (err) {
                                    res.render("doctors-listing.hbs", {
                                      title: "Doctors | HealthAura",
                                      message:
                                        "Something went wrong try to refress",
                                    });
                                  } else {
                                    let iterator = page - 5 < 1 ? 1 : page - 5;
                                    let endingLink =
...
exports.singleDoctorPage = (req, res) => {
  pool.getConnection((err, connection) => {
    if (err) throw err;
    connection.query(
      "SELECT * FROM healthaura_doctors WHERE id = ?",
      [req.params.id],
      (err, singleDoctorInfo) => {
        if (err) {
          res.redirect("/doctor");
        } else {
          res.render("single-doctor.hbs", {singleDoctorInfo, title: `Dr.${singleDoctorInfo[0].doctorName} | HealthAura` });
        }
      }
    );
  });
    ...
    if (err) {
      ...
      if (userToken) {
        verify(userToken, process.env.SECRET_KEY, (err, decoded) => {
          if (err) {
            connection.query(
              "SELECT * FROM healthaura_doctors WHERE specialities LIKE?",['%'+treatmentName+'%'],
              (err, allDoctorsData) => {
                if (err) {
                  res.render("doctors-listing.hbs", {
                    title: "Doctors | HealthAura",
                  ...
                  if (allDoctorsData.length == 0) {
                    ...
                    const startingLimit = (page - 1) * resultPerPage;
                    connection.query(
                      `SELECT * FROM healthaura_doctors WHERE specialities LIKE? LIMIT ${startingLimit},${resultPerPage}`,['%'+treatmentName+'%'],
                      (err, allDoctorsData) => {
                        if (err) {
                          res.render("doctors-listing.hbs", {
                            title: "Doctors | HealthAura",
